...
impl Umfrage {
    pub fn result2(&self) -> Vec<(UFrage, Vec<(UAntwort, Vec<Umfrageantwort>)>)> {
        let ufragen = UFrage::get_by_umfrage(self.id);
...
impl UFrage {
    pub fn get_by_umfrage(id: i32) -> Vec<UFrage> {
        ...
        umfrageufrage.inner_join(umfrage).inner_join(ufrage).filter(umfrageid.eq(id)).select(UFrage::as_select()).load::<models::UFrage>(conn).unwrap()
    }
}
